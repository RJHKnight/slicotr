sb02od_n <- function(dico, uplo, jobl, sort, n, m, a, b, q, r, l, tol, ldwork) {
    ...
    res <- .Fortran("SB02OD", DICO = dico, UPLO = uplo, JOBL = jobl, SORT = sort, N = n, M = m, A = a, B = b, Q = q, R = r, L = l, TOL = tol, LDWORK = ldwork, RCOND = rcond, X = x, ALFAR = alfar, ALFAI = alfai, BETA = beta, S = s, T = t,
        INFO = info, JOBB = jobb, FACT = fact, P = p, LDA = lda, LDB = ldb, LDQ = ldq, LDR = ldr, LDL = ldl, LDX = ldx, LDS = lds, LDT = ldt, U = u, IWORK = iwork, DWORK = dwork, BWORK = bwork, LDU = ldu)
