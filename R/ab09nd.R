ab09nd <- function(dico, job, equil, ordsel, n, m, p, nr, alpha, a, b, c, d, tol1, tol2, ldwork) {
    ...
    res <- .Fortran("AB09ND", DICO = dico, JOB = job, EQUIL = equil, ORDSEL = ordsel, N = n, M = m, P = p, NR = nr, ALPHA = alpha, A = a, B = b, C = c, D = d, TOL1 = tol1, TOL2 = tol2, LDWORK = ldwork, NS = ns, HSV = hsv, IWARN = iwarn,
        INFO = info, LDA = lda, LDB = ldb, LDC = ldc, LDD = ldd, IWORK = iwork, DWORK = dwork)
