...
sb10fd <- function(n, m, np, ncon, nmeas, gamma, a, b, c, d, tol, ldwork) {
    ...
    res <- .Fortran("SB10FD", N = n, M = m, NP = np, NCON = ncon, NMEAS = nmeas, GAMMA = gamma, A = a, B = b, C = c, D = d, TOL = tol, LDWORK = ldwork, AK = ak, BK = bk, CK = ck, DK = dk,
        RCOND = rcond, INFO = info, LDA = lda, LDB = ldb, LDC = ldc, LDD = ldd, LDAK = ldak, LDBK = ldbk, LDCK = ldck, LDDK = lddk, IWORK = iwork, DWORK = dwork, BWORK = bwork)
