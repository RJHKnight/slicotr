sg02ad_bn <- function(dico, uplo, jobl, scal, sort, acc, n, m, a, e, b, q, r, l, tol, ldwork) {
    ...
    res <- .Fortran("   FORTRANNAME SG02AD", DICO = dico, UPLO = uplo, JOBL = jobl, SCAL = scal, SORT = sort, ACC = acc, N = n, M = m, A = a, E = e, B = b, Q = q, R = r, L = l, TOL = tol, LDWORK = ldwork, RCONDU = rcondu, X = x, ALFAR = alfar,
        ALFAI = alfai, BETA = beta, S = s, T = t, U = u, IWARN = iwarn, INFO = info, JOBB = jobb, FACT = fact, P = p, LDA = lda, LDE = lde, LDB = ldb, LDQ = ldq, LDR = ldr, LDL = ldl, LDX = ldx, LDS = lds, LDT = ldt, LDU = ldu, IWORK = iwork,
