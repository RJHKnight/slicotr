ab08nd <- function(equil, n, m, p, a, b, c, d, tol, ldwork) {
    ...
    res <- .Fortran("AB08ND", EQUIL = equil, N = n, M = m, P = p, A = a, B = b, C = c, D = d, TOL = tol, LDWORK = ldwork, NU = nu, RANK_BN = rank_bn, DINFZ = dinfz, NKROR = nkror, NKROL = nkrol, INFZ = infz, KRONR = kronr, KRONL = kronl,
        AF = af, BF = bf, INFO = info, LDA = lda, LDB = ldb, LDC = ldc, LDD = ldd, LDAF = ldaf, LDBF = ldbf, IWORK = iwork, DWORK = dwork)
